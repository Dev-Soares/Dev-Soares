...
# Altura aumentada para 450 para dar margens externas generosas
WIDTH, HEIGHT = 850, 450 
# Centro deslocado para o meio absoluto do novo espaço, dando respiro para o topo
CENTER_X, CENTER_Y = 425, 225 
# Reduzido para 170 para que a galáxia fique bem longe das bordas laterais
MAX_RADIUS = 170 
SPIRAL_TURNS = 0.85
NUM_POINTS = 30
# Escalas ajustadas para centralizar o desenho
...
def _build_starfield(username, width, height, theme):
    """Build star depth layers with more padding from edges."""
    star_layers = [
...
def _build_tech_labels(galaxy_arms, arm_colors, all_arm_points, cx, cy):
    """Build labels with massive 35px padding from the arms."""
    arm_dots = []
    for arm_idx, arm in enumerate(galaxy_arms):
        ...
        for i, item in enumerate(items):
            ...
            # Distância aumentada para 35px para garantir que nada encoste
...
def _build_galaxy_core(cx, cy, theme, initial):
    """Build the central core with increased scaling."""
    return (
...
def render(config: dict, theme: dict, galaxy_arms: list, projects: list) -> str:
    """Main render function for the galaxy header."""
  ...
  <defs>
    <style>
      ...
      .core-glow {{ animation: pulse 4s infinite; }}
  ...
  <text x="{CENTER_X}" y="70" text-anchor="middle" fill="{theme['text_bright']}" font-size="36" font-weight="300" font-family="sans-serif">{esc(name)}</text>
